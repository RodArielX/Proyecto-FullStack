...
const TablaCompras = () => {
    ...
    const listarCompras = async () => {
        try {
            ...
            const url = `${import.meta.env.VITE_BACKEND_URL}/compra/listar`;
            const options = {
                headers: {
    ...
    const comprasFiltradas = compras.filter((c) => {
        const coincideCliente = c.nombreCliente?.toLowerCase().includes(filtroCliente.toLowerCase());
        const coincidePago = filtroMetodoPago ? c.metodoPago === filtroMetodoPago : true;
                    ...
                    <table className="min-w-full bg-zinc-900 text-white shadow-md rounded-xl overflow-hidden">
                        ...
                        <tbody>
                            {comprasFiltradas.map((compra, index) => (
                                <tr key={compra._id} className="border-b border-zinc-800 hover:bg-zinc-800">
                                    <td className="p-3">{index + 1}</td>
                                    <td className="p-3">{compra.nombreCliente}</td>
                                    <td className="p-3">${compra.total}</td>
                                    <td className="p-3">{compra.metodoPago}</td>
                                    ...
                                    <td className="p-3 flex justify-center">
                                        <Eye
                                            ...
                                            onClick={() => navigate(`/dashboard/visualizarCompra/${compra._id}`)}
                                        />
                                    </td>
                                </tr>
                            ))}
